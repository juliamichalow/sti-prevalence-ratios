...
mytheme <- theme_bw(base_size = 14) +
  theme(panel.grid = element_blank(),
        ...
        legend.margin = margin(unit(c(t=-5,r=10,b=0,l=10), "cm")),
        ...
        strip.text = element_text(color="black", size = rel(1.3), face="bold",
                                  margin = margin(unit(c(t=4,r=0,b=8,l=2), "cm")),
                                  hjust = 0, vjust = 0.5),
...
p1 <- df1 |>
  ...
  geom_linerange(aes(group = sex), position = position_dodge(0.9), size = 0.6) +
  geom_label(aes(label = paste0(sprintf("%.1f",prev*100)), y = prev + 0.01),
             position = position_dodge(0.9),
             size = 14/.pt, label.size = NA, 
...
p2 <- df2 |>
  ...
  geom_point(data = df_plot, aes(x = year_mid, y = adj_prev, colour = test, shape = sex), size = 3, fill = "white") +
  ...
  ggh4x::facet_wrap2(dplyr::vars(sti,region),
                     strip = ggh4x::strip_nested(text_x = ggh4x::elem_list_text(face = c("bold", "plain"),
                                                                                size = c(14*1.3,14*1.18)), # align with theme
                                                 by_layer_x = TRUE),
                     scales = "free_y",
                     nrow = 3,
                     axes = "x") +
...
(p1 / p2) + plot_layout(height = c(1,3.8))

ggsave("./plots/fig_2_large.png", width = 36, height = 50, unit = "cm", dpi = 600)  
