...
df <- df_obs |> filter(sex == "Female") |>
  ...
  filter(!year_mid %in% c(2000:2004))
...
rbind(df_adj |> mutate(type = "Adjusted"),
  ...
  geom_ribbon(aes(x = year, ymin = lwr, ymax = upr, fill = type), alpha = 0.15) +
  geom_line(aes(x = year, y = prev, colour = type), size = 0.6) +
  geom_point(data = df_obs |> filter(sex == "Female", year_mid >= 2005) |> mutate(type = "Adjusted"), 
             aes(x = year_mid, y = adj_prev, colour = type), size = 1) +
  geom_point(data = df_obs |> filter(sex == "Female", year_mid >= 2005) |> mutate(type = "Unadjusted"), 
...
ggsave("./plots/fig_supp_sensitivity_3.png", width = 18, height = 12, unit = "cm", dpi = 700)  
...
# model estimates
df_2005 <- read.csv("./results/prevalence_alldata_adjusted.csv") 
df_2000 <- read.csv("./results/prevalence_alldata_2000onwards.csv")

df_est <- rbind(df_2000 |> filter(sex == "Female") |> mutate(type = "2000 - 2023"),
                df_2005 |> filter(sex == "Female") |> mutate(type = "2005 - 2023")) |>
...
ggplot() +
  geom_ribbon(data = df_est,
              aes(x = year, ymin = lwr, ymax = upr, fill = type, alpha = "Extrapolated")) +
  geom_ribbon(data = df_est |>
                filter(!(type == "2005 - 2023" & year %in% c(2000:2004))),
              aes(x = year, ymin = lwr, ymax = upr, fill = type, alpha = "Interpolated")) +
  geom_line(data = df_est,
            aes(x = year, y = prev, colour = type, linetype = "Extrapolated"), size = 0.6) +
  geom_line(data = df_est |>
              filter(!(type == "2005 - 2023" & year %in% c(2000:2004))),
            aes(x = year, y = prev, colour = type, linetype = "Interpolated"), size = 0.6) +
  scale_fill_manual(values = colour_4) +
  scale_colour_manual(values = colour_4) +
  scale_alpha_manual("", values = c("Interpolated" = 0.1,"Extrapolated" = 0.06),
                     breaks = c("Interpolated","Extrapolated")) +
  scale_linetype_manual("", values = c("Interpolated" = "solid","Extrapolated" = "dotted"),
                        breaks = c("Interpolated","Extrapolated")) +
  guides(colour = guide_legend(order=2, title = "Estimate year range"),
         fill = "none",
         linetype=guide_legend(order=3, title = "Estimate type"),
         alpha=guide_legend(order=3, title = "Estimate type")) +
  # points on new colour scale
  ggnewscale::new_scale_color() +
  geom_point(data = df, aes(x = year_mid, y = adj_prev, colour = group), size = 1) +
  scale_colour_manual(values = colour_3) +
  guides(colour=guide_legend(order=1, title = "Study mid-year range")) +
  ggh4x::facet_wrap2(dplyr::vars(sti,region),
                     strip = strip_nested(text_x = elem_list_text(face = c("bold", "plain")),
                                          by_layer_x = TRUE),
                     scales = "free_y",
...
ggsave("./plots/fig_supp_sensitivity_4.png", width = 18, height = 20, unit = "cm", dpi = 700)  
